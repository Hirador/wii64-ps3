...
void RecompCache_Alloc(unsigned int size, unsigned int address, PowerPC_func* func){
	...
	void* code = heapAllocate(cache, size);
	while(!code){
		release(size);
		code = heapAllocate(cache, size);
	}
...
void RecompCache_Realloc(PowerPC_func* func, unsigned int new_size){
	...
	heapFree(cache, func->code);
	func->code = heapAllocate(cache, new_size);
	while(!func->code){
		release(new_size);
		func->code = heapAllocate(cache, new_size);
	}
...
void* MetaCache_Alloc(unsigned int size){
	void* ptr = heapAllocate(meta_cache, size);
	// While there's no room to allocate, call release
	while(!ptr){
		release(size);
		ptr = heapAllocate(meta_cache, size);
	}
